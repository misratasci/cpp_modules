...
Form::Form(const Form &src)
    : name(src.getName()), is_signed(src.getIsSigned()),
...
Form &Form::operator=(const Form &src) {
  std::cout << "Copy assignment operator for Form" << this->getName()
            << std::endl;
  if (this == &src)
    return *this;
...
std::ostream &operator<<(std::ostream &os, Form const &src) {
  os << src.getName() << ", with sign grade " << src.getSignGrade()
     << " and exec grade " << src.getExecGrade() << " is ";
  if (!src.getIsSigned())
    os << "not ";
  os << "signed";
  return (os);
